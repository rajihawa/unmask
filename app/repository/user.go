...
func NewUsersRepository() domain.UsersRepository {
	return &UsersRepository{
		db:   r.Table(utils.UsersTableName),
		sess: utils.Session,
	}
}
	...
	if opts.GetClient {
		term = term.Merge(func(p r.Term) interface{} {
			return map[string]interface{}{
				"client_id": r.Table(utils.ClientsTableName).Get(p.Field("client_id")),
			}
		})
	} else {
...
func (u *UsersRepository) Get(id string, projectID string, opts domain.GetUsersOpts) (*domain.User, error) {
	term := u.db.Get(id)
	term = filterUserOpts(term, opts)
	cur, err := term.Run(u.sess)
	if err != nil {
